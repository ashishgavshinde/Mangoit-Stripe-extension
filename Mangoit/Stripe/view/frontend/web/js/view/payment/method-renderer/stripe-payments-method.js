...
define(
    [
        ...
        'Mangoit_Stripe/js/model/payment/messages',
        'Magento_Checkout/js/model/payment/additional-validators',
        'Mangoit_Stripe/js/model/credit-card-validation/validator'
    ],
    function (Component, $, quote,customer, fullScreenLoader, redirectOnSuccessAction, messageContainer, additionalValidators) {
        ...
        return Component.extend({
            defaults: {
                template: 'Mangoit_Stripe/payment/stripe-payments-method',
                token: '',
                isSave: window.Mangoit.stripe.isSave,
                hasCard: (window.Mangoit.stripe.hasCard>0),
                id: '',
                ...
                if (self.getVal() === "0") {
                    ...
                    if (this.validate()) {
                        ...
                        Stripe.source.create({
                            type: 'card',
                            card: {
                                number: $('#Mangoit_stripe_cc_number').val(),
                                cvc: $('#Mangoit_stripe_cc_cid').val(),
                                exp_month: $('#Mangoit_stripe_expiration').val(),
                                exp_year: $('#Mangoit_stripe_expiration_yr').val()
                            },
                            owner: {
                                address: {
            ...
            addOption: function (id) {
                var checkFlag = window.Mangoit.stripe.hasCard;
                var cards = $.parseJSON(window.Mangoit.stripe.cards);
                if (checkFlag>0)
                {
                    var i;
            ...
            validate: function() {
                var self = this;
                if(window.Mangoit.stripe.publishableKey===""){
                    self.messageContainer.addErrorMessage({
            ...
            getInstructions: function () {
                return window.Mangoit.stripe.instructions;
            },
            ...
            afterPlaceOrder: function () {
                var self = this;
                $.ajax({
                    url: window.Mangoit.stripe.threedSecureUrl,
                    dataType: "json",
                    type: 'POST',
