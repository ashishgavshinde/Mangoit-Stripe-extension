...
define([
    ...
    window.StripeOrder = new Class.create();

    StripeOrder.prototype = {
        ...
        submit : function()
        {
            function createToken() {
                var firstName = jQuery("#order-billing_address_firstname");
                var lastName = jQuery("#order-billing_address_lastname");
                Stripe.card.createToken({
                    number: jQuery('#Mangoit_stripe_cc_number').val(),
                    cvc: jQuery('#Mangoit_stripe_cc_cid').val(),
                    exp_month: jQuery('#Mangoit_stripe_expiration').val(),
                    exp_year: jQuery('#Mangoit_stripe_expiration_yr').val(),
                    name: firstName + " " + lastName,
                    address_line1: jQuery("#order-billing_address_street0").val(),
                    address_city: jQuery("#order-billing_address_city").val(),
                    address_state: jQuery("#order-billing_address_region_id").val(),
                    address_zip: jQuery("#order-billing_address_postcode").val(),
                    address_country: jQuery("#order-billing_address_country_id").val()
                    ...
                    if (response.error) {
                        ...
                        jQuery("#Mangoit_stripe_token").val(response.id);
                        console.log("done");
            ...
            jQuery.when(createToken()).done(function(){
