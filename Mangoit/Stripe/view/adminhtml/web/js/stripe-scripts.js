...
define([
    ...
    $('#btn_tokenizer').on("click", function (e) {
        ...
        e.stopPropagation();
        var form = $('#edit_form'),
            ccNumber =  form.find("#Mangoit_stripe_cc_number"),
            ccCid = form.find("#Mangoit_stripe_cc_cid");
        var token;
        ...
        Stripe.card.createToken({
            ...
            if (response.error) {
                ...
                form.find('#Mangoit_stripe_token').val("");
                form.find('#Mangoit_stripe_cc_number').val("");
                form.find('#Mangoit_stripe_cc_cid').val("");
            } else {
                $('#text_warning').text("Card validated");
                token = response.id;
                //console.log(token);
                form.find('#Mangoit_stripe_token').val(token);
                // form.find('#Mangoit_stripe_cc_number').val("****************");
                // form.find('#Mangoit_stripe_cc_cid').val("****");
            }
        });
